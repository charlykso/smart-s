...
exports.getPrincipalDashboard = async (req, res) => {
  try {
    ...
    if (principalSchool) {
      ...
      financialOverview.totalRevenue = schoolPayments.reduce((sum, payment) => sum + payment.amount, 0);
      ...
      financialOverview.thisMonthRevenue = thisMonthPayments.reduce((sum, payment) => sum + payment.amount, 0);
    }
    ...
    if (principalSchool) {
      ...
      const recentPayments = await Payment.find({
        fee: { $in: await Fee.find({ school: principalSchool._id }).distinct('_id') },
        status: 'success'
      })
      .sort({ trans_date: -1 })
      .limit(3)
